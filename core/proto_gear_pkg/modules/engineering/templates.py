...
# ---------------------------------------------------------------------------
# Scaffold file taxonomy — single source of truth for what init produces.
#
# The wizard used to hardcode file lists in several places (summary preview,
# installation table, missing-template detection), and those lists drifted from
# what setup_agent_framework_only actually writes: SESSION_HANDOFF.md,
# AGENT_CONTEXT.md and the four host-config mirrors were created but never
# shown. These constants are the one list every display path consults so the
# wizard can no longer under-report the scaffold.
# ---------------------------------------------------------------------------

# Always written on init regardless of template selection.
CORE_ALWAYS_FILES = ["AGENTS.md", "SESSION_HANDOFF.md", "PROJECT_STATUS.md"]

# Auto-generated by sync_context; NOT user-selectable templates. AGENT_CONTEXT.md
# is the canonical agent skim; the rest mirror its managed block. Mirrors
# module_core.sync_context.HOST_FILES (source of truth for the host mirrors).
...
# Optional templates the user may select in the custom/preset flows.
...
# Template *names* (no .md) that must never appear as user-selectable choices,
# because they are always written or regenerated by sync_context.
