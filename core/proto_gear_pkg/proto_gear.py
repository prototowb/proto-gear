...
def setup_agent_framework_only(
    ...
    if not dry_run:
        try:
            ...
            # Write PROJECT_SPECIFICATIONS.md stub from the captured intent
            # (description, boundaries, conventions — ADR-004 planning intake).
            # Never clobber an existing specs file: intent captured anyway
            # still lands in the seed lesson and the handoff task below.
            intent_captured = bool(project_description or boundaries or conventions)
            specs_dest = current_dir / "PROJECT_SPECIFICATIONS.md"
            specs_written = False
            if intent_captured and not specs_dest.exists():
                from .modules.engineering.init_planning import build_specs_stub

                stub_content = build_specs_stub(
            ...
            if not handoff_file.exists():
                # Init ends by writing a task, not by pretending completeness
                # (ADR-004 move 3): captured intent becomes the first agent task.
                from .modules.engineering.init_planning import build_handoff_pending

                handoff_pending = build_handoff_pending(
                    intent_captured=intent_captured, specs_written=specs_written
                )
...
"""
            ...
            # Priority 1: Use core_templates if provided (from wizard). An
            # explicitly-passed empty selection means "no extra templates" —
            # it must not fall through to the with_branching legacy branch,
            # or init would write files the accepted plan never showed
            # (PROTO-100: the detected plan is a contract).
            ...
            # Seed a first lesson from captured boundaries/conventions (ADR-004):
            # the agent-writable lessons layer starts with the operator's stated
            # facts instead of empty. Only on a fresh lessons dir, never clobber.
            if boundaries or conventions:
                from .modules.engineering.init_planning import build_seed_lesson

                lessons_dir = current_dir / ".proto-gear" / "lessons"
                seed_path = lessons_dir / "house-conventions.md"
                if lessons_dir.exists() and not seed_path.exists():
                    seed_content = build_seed_lesson(
                        boundaries=boundaries, conventions=conventions
                    )
                    try:
                        seed_path.write_text(seed_content, encoding="utf-8")
                        files_created.append(".proto-gear/lessons/house-conventions.md")
                        from .module_core import lessons as lessons_module

                        lessons_module.sync_lessons_index(
                            current_dir / ".proto-gear", dry_run=False
                        )
                        print(
                            f"{Colors.GREEN}+ Seed lesson written "
                            f"(.proto-gear/lessons/house-conventions.md){Colors.ENDC}"
                        )
                    except (IOError, OSError) as e:
                        print(
                            f"{Colors.YELLOW}Warning: Could not write seed lesson: {e}{Colors.ENDC}"
                        )
        ...
        # Priority 1: Use core_templates if provided (from wizard). Mirrors the
        # real path: an explicit empty selection never falls through to legacy.
        if core_templates is not None and isinstance(core_templates, dict):
            for template_name, should_generate in core_templates.items():
