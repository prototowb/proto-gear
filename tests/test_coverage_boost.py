...
def generate_project_template(template_name, project_dir, context, **kwargs):
    ...
    result = _generate_project_template_raw(
        template_name, project_dir, context, **kwargs
    )
    if result is None:
        return None
    path, _action = result
...
class TestGitConfigWorkflowModes:
    ...
    def test_detect_git_local_only(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            assert (
                result.get("workflow_mode") == "local_only"
                or result.get("has_remote") is False
            )
    ...
    def test_detect_git_remote_manual(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            assert (
                result.get("workflow_mode") in ["remote_manual", "remote_automated"]
                or result["has_remote"] is True
            )
...
class TestBranchingDocWorkflowModes:
    ...
    def test_branching_doc_remote_manual_mode(self):
        ...
        assert (
            "manual" in result.lower() or "web" in result.lower() or result is not None
        )
    ...
    def test_branching_doc_local_only_mode(self):
        ...
        assert (
            "local" in result.lower()
            or "remote" in result.lower()
...
class TestTemplateDiscovery:
    ...
    def test_generate_template_with_missing_template(self, tmp_path):
        ...
        result = generate_project_template(
            "COMPLETELY_NONEXISTENT_TEMPLATE_XYZ", tmp_path, {"PROJECT_NAME": "test"}
        )
...
class TestCapabilitySystem:
    ...
    def test_copy_capabilities_returns_dict(self, tmp_path):
        ...
        result = copy_capability_templates(
            tmp_path, project_name="test-proj", dry_run=True
        )
    ...
    def test_copy_capabilities_dry_run_no_files(self, tmp_path):
        ...
        copy_capability_templates(tmp_path, project_name="test-proj", dry_run=True)
    ...
    def test_copy_capabilities_with_specific_config(self, tmp_path):
        ...
        config = {"skills": ["tdd"], "workflows": ["feature-dev"], "commands": ["test"]}

        result = copy_capability_templates(
            tmp_path, project_name="test-proj", dry_run=True, capabilities_config=config
        )
